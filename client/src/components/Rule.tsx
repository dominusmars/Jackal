...
import { ExtendedSuricataRule } from "lib";
...
function Rule({ rule }: Props) {
    ...
    return (
        <>
            ...
            >
                <td className="max-w-6 text-wrap text-center border px-4 py-2">
                    {rule.action}
                </td>
                <td className="max-w-6 text-wrap text-center border px-4 py-2">
                    {rule.protocol}
                ...
                <tr className="bg-gray-300">
                    <td colSpan={8} className="p-5">
                        ...
                        <div className="text-gray-700 dark:text-gray-400 mb-2">
                            {Object.entries(rule.options).map(
                                ([key, value], i) => (
                                    <p key={i}>
                                        <strong>{key}:</strong>{" "}
                                        {value.toString()}
