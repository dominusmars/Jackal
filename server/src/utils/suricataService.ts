import { SuricataConfig, SuricataInterface, ExtendedSuricataRule, SuricataRule, SuricataCaptureType, SuricataDPDKInterface } from "lib/suricata";
...
class Suricata {
    ...
    async addInterface(networkInterface: SuricataInterface, captureType: SuricataCaptureType) {
        let config = this.getSuricataConfig();
        if (captureType === "dpdk") {
            config[captureType].interfaces.push(networkInterface);
        } else config[captureType].push(networkInterface);
        this.writeSuricataConfig(config);
    }
    async removeInterface(networkInterface: SuricataInterface, captureType: SuricataCaptureType) {
        ...
        else config[captureType] = config[captureType].filter((iface) => iface.interface !== networkInterface.interface);
        this.writeSuricataConfig(config);
    }
