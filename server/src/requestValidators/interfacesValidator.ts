...
export const SuricataAFInterfaceValidator = Joi.object({
    interface: Joi.string().required(),
    "cluster-id": Joi.number().optional(),
    "cluster-type": Joi.string()
        .valid("cluster_flow", "cluster_cpu", "cluster_qm", "cluster_ebpf")
        .optional(),
    defrag: Joi.boolean().optional(),
    threads: Joi.alternatives()
        .try(Joi.string().valid("auto"), Joi.number())
        .optional(),
    ...
    "checksum-checks": Joi.string()
        .valid("kernel", "yes", "no", "auto")
        .optional(),
...
export const SuricataXDPInterfaceValidator = Joi.object({
    interface: Joi.string().required(),
    threads: Joi.alternatives()
        .try(Joi.string().valid("auto"), Joi.number())
        .optional(),
...
export const SuricataDPDKInterfaceValidator = Joi.object({
    interface: Joi.string().required(),
    threads: Joi.alternatives()
        .try(Joi.string().valid("auto"), Joi.number())
        .required(),
    ...
    "mempool-cache-size": Joi.alternatives()
        .try(Joi.number(), Joi.string().valid("auto"))
        .required(),
...
export const SuricataPcapInterfaceValidator = Joi.object({
    interface: Joi.string().required(),
    "buffer-size": Joi.number().optional(),
    "bpf-filter": Joi.string().optional(),
...
export const SuricataCaptureTypeValidator = Joi.string().valid(
    "af-packet",
    "af-xdp",
    "dpdk",
    "pcap"
